...
{
	unsigned int ESP_Msg_Begin = 0 , ESP_Msg_End = 0;
    ...
    while(Serial3.available())                                                                     // Is data available from esp8266
        {
            ...
            Rdata=Serial3.read();
            Serial.print(Rdata);
            received_data = received_data + Rdata; 
        }
	
	ESP_Msg_Begin = received_data.indexOf("{");
	ESP_Msg_End = received_data.indexOf("}");
		
	if((ESP_Msg_Begin>=0)&&(ESP_Msg_End>0)&&(ESP_Msg_End>ESP_Msg_Begin)&&(ESP_Msg_Begin<STRING_LENGTH_OVERFLOW_VALUE)&&(ESP_Msg_End<STRING_LENGTH_OVERFLOW_VALUE))   // IS the Received data beginning with "{" ending with "}"
		{                        
    ...
    if (msgReceived == true)
		{                                                                                            // input format {"message": 1}
			...
			received_data = "";                               // If after message received and string is not cleared then clear the buffer.
			Serial3.flush();
			Serial.flush();
		}
	//  Serial.println("---------------------BEG--------------------------------------");
	//  Serial.println("received_data:"); Serial.println(received_data);
	//  Serial.println("---------------------END--------------------------------------");
	  
	Received_String_Length = received_data.length();
	...
	if (Received_String_Length > MAX_LENGTH_OF_STRING)                              // is the string length more than 100? that means it is about to overflow and it cannot take new messages...
		received_data = "";                                                        // clear the input string ; to let the *.indexOf() function work.
